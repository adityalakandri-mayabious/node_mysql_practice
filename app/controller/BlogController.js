  ...
  try {
    ...
    if (newImages && newImages.length > 0) {
      for (let img of newImages) {
        ...
        await Blog.query(`INSERT INTO blog_image(image,blog_id) VALUES(?,?)`, [
          imagePath,
          id,
        ]);
      }
...
export const deleteBlog = async (req, res) => {
  try {
    ...
    const [blogImages] = await Blog.query(
      `SELECT * FROM blog_image WHERE blog_id = ?`,
      [id]
    );
    if (blogImages.length > 0) {
      for (let img of blogImages) {
        ...
        const fullpath = path.join(__dirname, "..", "..", img.image);
      ...
      await Blog.query(`DELETE FROM blog_image WHERE blog_id=?`, [id]);
    }
